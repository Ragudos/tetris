...
export default class SwapRenderer extends PIXI.Container {
	...
	constructor(app: PIXI.Application, type_of_sprite: Sprites, game: Game) {
		...
		this.position.set(
			app.screen.width / 2 -
				(config.display.width + config.display.side_width / 2),
			app.screen.height / 2 -
				(config.display.height - config.display.height / 1.675),
		);
		...
		this.__graphics.drawRect(
			0,
			0,
			config.display.side_width * 1.5,
			config.display.side_width,
		);
	...
	draw(): void {
		...
		const texture = PIXI.Assets.cache.get(
			...
			for (let x = 0; x < blocks[tetromino.name]![y]!.length; ++x) {
				...
				if (tetromino.name === "I") {
					sprite.sprite.position.set(
						sprite.sprite.width * x +
							config.display.side_width / 2 -
							sprite.sprite.width,
						config.display.side_width / 2 -
					...
					sprite.sprite.position.set(
						sprite.sprite.width * x +
							(config.display.side_width * 1.5) / 2 -
							sprite.sprite.width,
						sprite.sprite.height * y +
							config.display.side_width / 2 -
					...
					sprite.sprite.position.set(
						sprite.sprite.width * x +
							(config.display.side_width * 1.25) / 2 -
							sprite.sprite.width,
						sprite.sprite.height * y +
							config.display.side_width / 2 -
							sprite.sprite.height,
